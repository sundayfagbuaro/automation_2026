r"""
Copyright &copy; 2026 NetApp Inc.
All rights reserved.

This file has been automatically generated based on the ONTAP REST API documentation.

"""
from marshmallow import EXCLUDE, fields as marshmallow_fields  # type: ignore
from netapp_ontap.resource import Resource, ResourceSchema, ResourceSchemaMeta, ImpreciseDateTime, Size, lazy_import_schema


__all__ = ["DcnHardwareCpu", "DcnHardwareCpuSchema"]
__pdoc__ = {
    "DcnHardwareCpuSchema.resource": False,
    "DcnHardwareCpuSchema.opts": False,
    "DcnHardwareCpu": False,
}

class DcnHardwareCpuSchema(ResourceSchema, metaclass=ResourceSchemaMeta):
    """The fields of the DcnHardwareCpu object"""

    count = Size(data_key="count", allow_none=True)
    r""" Number count of CPUs on the node.

Example: 20 """

    firmware_release = marshmallow_fields.Str(data_key="firmware_release", allow_none=True)
    r""" Firmware release number. Defined by the CPU manufacturer. """

    memory_size = Size(data_key="memory_size", allow_none=True)
    r""" GPU Memory available on the node, in bytes.

Example: 1024000000 """

    processor = marshmallow_fields.Str(data_key="processor", allow_none=True)
    r""" CPU type on the node. """

    @property
    def resource(self):
        return DcnHardwareCpu

    gettable_fields = [
        "count",
        "firmware_release",
        "memory_size",
        "processor",
    ]
    """count,firmware_release,memory_size,processor,"""

    patchable_fields = [
    ]
    """"""

    postable_fields = [
    ]
    """"""


class DcnHardwareCpu(Resource):

    _schema = DcnHardwareCpuSchema
